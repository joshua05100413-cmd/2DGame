...
>
> 实测过一次：两个窗口同时开着，房主端状态栏写着
> `已开房 SteamID 76561199372121221`，加入端把**同一个数字**填进地址栏 ——
> 于是它在连自己。Steam 拒绝自己连自己，加入端一直卡在 CONNECTING 直到超时。
>
> 现在这种情况会被**当场挡下**并提示「不能连接到自己」，不再白等 20 秒：
>
> ```
> 连接失败：不能连接到自己：76561199372121221 就是本机登录的 Steam 账号。
> Steam 联机需要两个不同的账号、各一台机器 —— 同一台电脑双开共用同一个
> SteamID，Steam 不允许自己连自己。一台电脑自测请改用 ENet 后端。
> ```
