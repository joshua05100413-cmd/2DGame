...
| `_spawn_pickup` / `_despawn_pickup` | 房主 → 全体 | 掉落物生成 / 移除 |
| `_report_pickup_claimed` | 客户端 → 房主 | 上报「我捡到了」，房主裁定归属 |
| `_report_shot` / `_shot_fired` | 客户端 ⇄ 房主 → 全体 | 开火表现（房主中转） |
...
- 玩家位置与朝向、玩家名单与准备状态
- 怪物生成 / 位置 / 血量 / 死亡
- 怪物受伤（客户端上报 → 房主结算 → 两端血量一致）
- **玩家受伤**（房主判定怪物攻击 → 通知对应玩家的本机扣血）
- **掉落物（金币）生成与拾取归属**
- **开火表现**（队友能看到你的弹道）
- 关卡号 / 剩余时间 / 击杀 / 金币
...
- 装备与商店（`EquipOnFloor`、`ShopPanel`）、奖励选择（`RewardChoose`）：
  都是各端各自的本地事件，队友看不到你丢在地上的枪。
- 其它消耗品掉落：只有金币接了 Coop 复制，`HpPack` 之类的道具还没有。
- 玩家经验与升级：只有击杀者所在端会加经验。
- 怪物攻击的表现细节（击退、特效）在客户端由影子本地播放，可能与房主不完全一致。
...
补玩法同步时最容易出错的是「这个东西该用哪种语义」。现成的三种是：

| 实体 | 客户端上的形态 | 谁决定生成/消失 | 交互方式 |
| --- | --- | --- | --- |
| 怪物 | **影子**：不跑 AI、不本地结算 | 房主 | 客户端命中只上报伤害 |
| 掉落物 | **实体**：可被本地玩家碰到 | 房主决定生成，房主裁定归属 | 本地拾取后上报 |
| 表现子弹 | **纯表现**：碰撞层为空 | 各端本地生成 | 不参与任何判定 |

关键差别在第二列：怪物在客户端**不能**可交互（否则各端各算一次伤害），
而掉落物在客户端**必须**可交互（否则只有房主能捡东西）。
新增可联网的玩法对象时，先想清楚它属于哪一类，再决定照着哪段代码写。
